...
GetURLHandler* GetURLHandler::Create(pp::Instance* instance,const std::string& url,NaClHTTPCallback callback,void *pUserData)
{
  return new GetURLHandler(instance,url,callback,pUserData);
}

GetURLHandler::GetURLHandler(pp::Instance* instance,const std::string& url,NaClHTTPCallback callback,void *pUserData)
    : instance_(instance),
      url_(url),
...
void GetURLHandler::ReportResult(const std::string& fname,
  ...
  */
  ...
  if(m_pCallback)
  {
	  m_pCallback(m_pUserData, this, success ? kHttpRequestOK : kHttpRequestCommsError, text.c_str());
  }
}
